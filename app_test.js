			...
			_.each(ids1, function (roomID) {
				...
				dbFunctions.getMessages(roomID, 0)
					.then(function (history) {
						...
						history.splice(-1, 1);
console.log('adminSocket.username',socket.username)
 ...
 */		//If some user is already online on chat
		...
		//	console.log('userSocket s',userSocket)
				...
				//		console.log(userSocket.roomID)
					...
					//	console.log(len);
						...
						history.splice(-1, 1);
				//	console.log("userSocket1",users[history[len-1]]);
					//
 	...
 	function user_IPaddress_roomID(c_roomId,clientip1){
		sequelize
		.query('exec user_IPaddress_roomID @room_Id='+c_roomId+',@ip_address='+clientip1+'')
      		.spread(records=>{
	console.log('record in user_IPaddress_roomID',records)
	...
	console.log('just before assignment checking the Usr_ID',Usr_Id)
			...
			//	setTimeout(() => {
				get_a_user_Admin_id(Usr_Id)
					//}, 20);
	 console.log('after executing the ticket assignment usr_|id',Usr_Id);
				 }
			   else{
	   console.log('No Tickets are there')
			   }
  ...
  console.log('after executing the ticket assignment usr_id',Usr_Id);
	...
	dbFunctions.getMessages(socket.roomID, 0)
		...
		/* 			 	setTimeout(() => {
						...
						c_roomId=socket.roomID;
						var c_roomId1=c_roomId.split('-').join("");
						setTimeout(() => {
							user_IPaddress_roomID(c_roomId1,clientip1)
