...
$('.msg_head').click(function() {
	console.log("clicked here at msg head",id);
	...
	if(id==null){
		...
		if (id != null && !active) {
			socket.emit('add user', {  //Id is there and he is active
				isNewUser: false,
				roomID: id
			});
		  $form.hide();
...
socket.on('chat message', function(data) {
	var sender;
	console.log('data in chat message client',data)
	if (data.isAdmin)
		sender = "msg_a"
	else
		sender = "msg_b"
...
socket.on('more chat history', function(data) {
	...
	for (var i = 0; i < len; i++)
		...
		console.log('moree chat history');
});
