...
console.log('n1',n1)
	...
	if (data.isAdmin){
	number1='msg_b';
	$(".content1").append('<div class= "' +number1 +'">'+data.msg+'<br>'+date.toDateString().fontsize(0.5)+'</div>');
	$('.inputMessage').val('');
	}
...
socket.on('New Client', function(data) {
	i++;
	console.log('data history',data['history']['what']);
...
/* 		$messages[0].scrollTop = $messages[0].scrollHeight;
			console.log(data['history'][i]['what'])
		}
	console.log('data in newclient',data)
...
socket.on('typing', function(data) {
	console.log('data in typing1',data)
 	$inputMessage = $('#' + data.roomID);
	/* var $parent = $inputMessage.parent();
	var $typing = $parent.children(".typing"); */
	if (data.isTyping)

		$keystroke.text("<small>" + data.person + " ...<small>");
	else
		$keystroke.text('');
})
...
function getChatArea(id) {
	return ("<ul class='messages'></ul><div class='t1'></div><input class='inputMessage' id='" + id + "'' placeholder='Type here...'  /></div>")
	//return ("</div><input class='inputMessage' id='" + id + "'' placeholder='Type here...'  /></div>");
...
socket.emit('chat message', {
	...
	timestamp: time
		  ...
		  number1="msg_b";
		$(".content1").append('<div class= "' +number1 +'">'+ii+'<br>'+date.toDateString().fontsize(0.5)+'</div>');
$('.inputMessage').val('');
		}else
