...
var $inputMessage;
var $formcontrol;// Input message input box
...
console.log('socket on session');
    ...
    console.log('else after session')
...
console.log('data in else data isadmin',data)

    var number2="msg_a";

    $(".content1").append('<div class= "' +number2 +'">'+data.msg+'<br>'+date.toDateString().fontsize(0.5)+'</div>');
	$('.formcontrol').val('');
}
...
/* 	if (!data.justJoined) {
	...
	$('.formcontrol').on('keypress', function() {
		isTyping();
	});
})
...
socket.on('typing', function(data) {
	//$inputMessage = $('#' + data.roomID);
	$formcontrol = $('#' + data.roomID);


	var $parent = $formcontrol.parent();
	var $typing = $parent.children(".typing");
	if (data.isTyping)
		$formcontrol.append("<small>" + data.person + " ...<small>");
	else
		$formcontrol.text('');
})
